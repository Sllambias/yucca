...
def main():
    ...
    parser.add_argument("-t", "--task", help="Name of the task to preprocess. "
    ...
    parser.add_argument("--ensemble", help="Used to initialize data preprocessing for ensemble/2.5D training", default=False,
                        action='store_true')
    parser.add_argument("--disable_unit_tests", help="Enable or disable unittesting",
                        default=False)
    parser.add_argument("--multi_task", help="Enable multi task preprocessing",
    ...
    if not ensemble:
        planner = recursive_find_python_class(folder=[join(yucca.__path__[0], 'planning')],
                                        class_name=planner_name, current_module='yucca.planning')
