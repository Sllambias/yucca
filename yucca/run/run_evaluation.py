...
def main():
    ...
    parser.add_argument("-s", help="Name of the source task i.e. what the model is trained on. "
                        "Should be of format: TaskXXX_MYTASK", required=False)
    ...
    parser.add_argument('--as_binary', help='run evaluation as if data was binary',action='store_true', default=None, required=False)
    ...
    if not predpath:
        ...
        predpath = join(yucca_results, target_task, source_task, model+dimensions, 
                        trainer_name + '__' + plan_id, "fold_" + folds + "_" + checkpoint)
    ...
    evaluator = YuccaEvaluator(classes, folder_with_predictions=predpath,
                              folder_with_ground_truth=gtpath, do_object_eval=obj,
                              as_binary=as_binary)
    evaluator.run()
