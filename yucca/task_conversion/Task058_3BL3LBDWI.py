...
for sTr in training_samples:
    ...
    for i in range(6):
        sitk.WriteImage(image_file, f"{target_imagesTr}/{prefix}_{sTr}_{str(i)}_000.nii.gz")
        sitk.WriteImage(label, f"{target_labelsTr}/{prefix}_{sTr}_{str(i)}.nii.gz")
...
for center in subdirs(train_folder, join=False):
    ...
    for sTr in training_samples:
        ...
        for i in range(6):
            sitk.WriteImage(image_file, f"{target_imagesTr}/{prefix}_{center+sTr}_{str(i)}_000.nii.gz")
            sitk.WriteImage(label, f"{target_labelsTr}/{prefix}_{center+sTr}_{str(i)}.nii.gz")
...
for dataset in datasets:
    ...
    for sTr in training_samples:
        ...
        flair_file = nibpro.resample_from_to(flair_file, mask, order=3)
        data = mask.get_fdata()
        data[data != 1] = 0
        data[data == 1] = 3
        ...
        for i in range(3):
            nib.save(flair_file, filename=f"{target_imagesTr}/{prefix}_{sTr}_{str(i)}_000.nii.gz")
            nib.save(mask, filename=f"{target_labelsTr}/{prefix}_{sTr}_{str(i)}.nii.gz")
...
hierarchy = {"Background": 0, "Lesion": {"WM": {"WMH": 3, "MS": 1}, "Stroke": 2}}
...
generate_dataset_json(
    ...
    target_imagesTs,
    ("Flair",),
    labels={0: "background", 1: "MS Lesion", 2: "Stroke", 3: "WMH"},
