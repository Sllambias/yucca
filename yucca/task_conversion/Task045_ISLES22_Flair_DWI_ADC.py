...
###OUTPUT DATA
# Target names
task_name = 'Task045_ISLES22_Flair_DWI_ADC'
prefix = 'ISLES22_FDWIADC'
...
for sTr in training_samples:
    ...
    flair_file = nibpro.resample_from_to(flair_file, dwi_file, order=3)

    adc_file = nib.load(join(images_dir, sTr, 'ses-0001', 'dwi', sTr+'_ses-0001_adc'+file_suffix))
    adc_file = nibpro.resample_from_to(adc_file, dwi_file, order=3)

    label = nib.load(join(labels_dir, sTr, 'ses-0001', sTr+'_ses-0001_msk'+file_suffix))
    label = nibpro.resample_from_to(label, dwi_file, order=0)
...
generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('Flair', 'DWI', 'ADC'),
