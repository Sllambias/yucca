...
path = "/home/zcr545/data/data/projects/PsyBrainPrediction/ADNI"
...
for _, row in tqdm(all_train[:10].iterrows(), total=len(all_train)):
    subject = row["Subject"]
    label = row["Group"]
...
for _, row in tqdm(all_test[:10].iterrows(), total=len(all_test)):
    subject = row["Subject"]
    label = row["Group"]
