...
class YuccaMultiTaskPreprocessor(YuccaPreprocessor):
    ...
    def _preprocess_train_subject(self, subject_id_and_task):
        ...
        imagepaths = [impath for impath in subfiles(join(self.imagedirs, task)) if os.path.split(impath)[-1].startswith(subject_id + '_')]
        image_props['image files'] = imagepaths
        images = [nib.load(image) for image in imagepaths]

        # Do the same with segmentation
        seg = [segpath for segpath in subfiles(join(self.labeldirs, task)) if os.path.split(segpath)[-1].startswith(subject_id + '.')]
