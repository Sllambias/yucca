...
class Masking(YuccaTransform):
    ...
    def __init__(
        ...
        mask=False,
        data_key="image",
        mask_ratio: Union[Iterable[float], float] = 0.25,
        mask_token_size: Union[Iterable[Union[int, float]], float, int] = 0.05,
    ):
        self.mask = mask
    ...
    def get_params(input_shape, mask_ratio, mask_token_size):
        ...
        if isinstance(mask_token_size, float):
            ...
            if mask_token_size > 0.25:
                logging.warn(
                    "mask_token_size is set to a ratio over 0.25 of the image. "
            ...
            # If we get a
            assert (
                len(mask_token_size) == 2 and np.all(1 > np.array(mask_token_size)) and np.all(np.array(mask_token_size) > 0)
            )
    ...
    def __call__(self, packed_data_dict=None, **unpacked_data_dict):
        ...
        if self.mask:
            mask_ratio, mask_token_size = self.get_params(
                data_dict[self.data_key].shape[2:], self.mask_ratio, self.mask_token_size
            )
