...
class YuccaPlanner(object):
    ...
    def preprocess(self):
        preprocessor = recursive_find_python_class(folder=[join(yucca.__path__[0], 'preprocessing')],
                                                   class_name=self.preprocessor,
                                                   current_module='yucca.preprocessing')

        preprocessor = preprocessor(self.plans_path, self.task, self.threads,
                                    self.disable_unittests)
        preprocessor.run()
        self.postprocess()
...
class YuccaPlannerX(YuccaPlanner):
    ...
    """
    def __init__(self, task, threads=2, disable_unittests=False):
        super().__init__(task, threads, disable_unittests)
