...
class YuccaLoader_Pad(YuccaLoader):
    ...
    """
    def __init__(self, list_of_files, batch_size,
                 gen_patch_size, p_oversample_foreground=1.,
                 random_state=None, keep_in_ram=True):
        super().__init__(list_of_files, batch_size, gen_patch_size,
                         p_oversample_foreground, random_state, keep_in_ram)
    ...
    def generate_3D_batch(self):
        ...
        for idx, sample in enumerate(samples):
            ...
            image = np.pad(imseg[:-1], ((0, 0),
                           (pad_lb_x, pad_ub_x),
                           (pad_lb_y, pad_ub_y),
                           (pad_lb_z, pad_ub_z)),
                           mode='edge')
            seg = np.pad(imseg[-1:], ((0, 0),
                         (pad_lb_x, pad_ub_x),
                         (pad_lb_y, pad_ub_y),
                         (pad_lb_z, pad_ub_z)),
                         mode='constant')
            ...
            if np.random.uniform() < self.p_oversample_foreground:
                ...
                for d in range(3):
                    ...
                    crop_start_idx += [np.random.randint(max(0, location[d] + offsets[d] - self.gen_patch_size[d]),
    ...
    def generate_2D_batch_from_3D(self):
        ...
        for idx, sample in enumerate(samples):
            ...
            if np.random.uniform() < self.p_oversample_foreground:
                ...
                x_idx = location[0]
                for d in range(2):
                    crop_start_idx += [np.random.randint(max(0, location[d+1] + offsets[d] - self.gen_patch_size[d]),
            ...
            image_batch[idx] = image[:, x_idx,
                                     crop_start_idx[0]: crop_start_idx[0] + self.gen_patch_size[0],
                                     crop_start_idx[1]: crop_start_idx[1] + self.gen_patch_size[1]]
            seg_batch[idx] = seg[:, x_idx,
                                 crop_start_idx[0]: crop_start_idx[0] + self.gen_patch_size[0],
                                 crop_start_idx[1]: crop_start_idx[1] + self.gen_patch_size[1]]
