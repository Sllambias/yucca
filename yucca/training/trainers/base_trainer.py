...
class base_trainer(object):
    def __init__(self):
        ...
        self.train_batches_per_epoch \
            ...
            = self.batch_size_2D = self.batch_size_3D \
            = self.folds = self.model_dimensions \
            = self.model_name = self.outpath \
            = self.patch_size_2D = self.patch_size_3D \
            = self.task = self.plan_id = self.name = None
        ...
        self.starting_lr \
            ...
            = self.fast_val_batches_per_epoch \
            = self.fast_final_epoch = None

        # These will always be set by the plans file
        self.classes = self.nclasses = self.modalities \
            = self.nmodalities = self.folder_with_preprocessed_data \
            = self.plans = self.plans_path = None
    ...
    def epoch_finish(self):
        ...
        wandb.log({"Training Loss": round(self.tr_losses[-1], 3),
        ...
        if round(self.val_losses[-1], 3) < self.best_val_loss:
            ...
            wandb.log({"Best Validation Loss": self.best_val_loss},
                      commit=False)
    ...
    def initialize_wandb(self):
        wandb.init(project="Yucca",
                   group=self.task,
                   dir=self.outpath,
                   name=join(os.path.split(self.outpath)[-1] + '_'
    ...
    def load_checkpoint(self, checkpoint, train=False):
        ...
        if self.finetune:
            ...
            state_dict = {k: v for k, v in chk['model_state_dict'].items()
        ...
        if 'patch_size' in chk:
            ...
            print("patch_size not found in checkpoint. Generating a new one now, better hope "
    ...
    def load_data(self):
        ...
        self.tr_loader = YuccaLoader(self.train_samples,
                                    self.batch_size,
                                    self.initial_patch_size,
                                    p_oversample_foreground=self.p_force_foreground)
        self.val_loader = YuccaLoader(self.val_samples,
                                     self.batch_size,
                                     self.patch_size,
                                     p_oversample_foreground=self.p_force_foreground)
    ...
    def get_data_generators(self):
        ...
        # Augmentations
        train_transforms.append(Spatial(crop=self.EarlyCrop,
        ...
        train_transforms.append(AdditiveNoise(p_per_sample=self.AdditiveNoise_p_per_sample,
                                              mean=self.AdditiveNoise_mean,
                                              sigma=self.AdditiveNoise_sigma))
        
        train_transforms.append(Blur(p_per_sample=self.Blurring_p_per_sample,
                                    p_per_channel=self.Blurring_p_per_channel,
                                    sigma=self.Blurring_sigma))
        
        train_transforms.append(MultiplicativeNoise(p_per_sample=self.MultiplicativeNoise_p_per_sample,
                                                    mean=self.MultiplicativeNoise_mean,
                                                    sigma=self.MultiplicativeNoise_sigma))
        
        train_transforms.append(MotionGhosting(p_per_sample=self.MotionGhosting_p_per_sample,
                                               alpha=self.MotionGhosting_alpha,
                                               numReps=self.MotionGhosting_numReps,
                                               axes=self.MotionGhosting_axes))
        
        train_transforms.append(GibbsRinging(p_per_sample=self.GibbsRinging_p_per_sample,
                                             cutFreq=self.GibbsRinging_cutFreq,
                                             axes=self.GibbsRinging_axes))
        
        train_transforms.append(SimulateLowres(p_per_sample=self.SimulateLowres_p_per_sample,
        ...
        train_transforms.append(Gamma(p_per_sample=self.Gamma_p_per_sample,
                                      p_invert_image=self.Gamma_p_invert_image,
                                      gamma_range=self.Gamma_range))

        train_transforms.append(Mirror(p_per_sample=self.Mirror_p_per_sample,
    ...
    def log(self, *args, time=True, also_print=True):
        if not self.log_file:
            ...
            self.log_file = join(self.outpath, 'log_' + strftime("%Y_%m_%d_%H_%M_%S",
                                                                 self.initial_time_obj) + '.txt')
    ...
    def preprocess_predict_test_case(self, case, outpath, save_softmax=False, overwrite=False,
                                     do_tta=False):
        """
        ...
        logits = self.network.predict(mode=self.model_dimensions,
                                      data=case, patch_size=self.patch_size,
        ...
        save_segmentation_from_logits(logits,
                                      outpath=outpath,
                                      properties=image_properties,
                                      save_softmax=save_softmax, compression=self.compression_level)
        print("\n")
    ...
    def set_outpath(self):
        if not self.outpath:
            ...
            self.outpath = join(yucca_models, self.task, self.model_name, self.model_dimensions,
    ...
    def set_random_seeds(self, seed=None):
        if not self.is_seeded:
            if seed is not None:
                ...
                self.log("No random seed using initial time object: ", self.random_seed, time=False)
