...
from yuccalib.evaluation.metrics import dice, sensitivity, precision, TP, FP, FN, total_pos_gt, total_pos_pred, volume_similarity
...
class YuccaEvaluator(object):
    def __init__(self, labels: list, folder_with_predictions, folder_with_ground_truth,
                 do_object_eval=False, as_binary=False):
        self.name = 'results'
        self.metrics = {'Dice': dice, 'Sensitivity': sensitivity,
                        'Precision': precision, 'Volume Similarity': volume_similarity,
                        'True Positives': TP,
        ...
        if do_object_eval:
            self.name += '_OBJ'
            self.obj_metrics = ['_OBJ Total Objects Prediction','_OBJ Total Objects Ground Truth',
                                '_OBJ True Positives', '_OBJ False Positives', '_OBJ False Negatives',
                                '_OBJ Mean Volume Prediction', '_OBJ Mean Volume Ground Truth',
                                '_OBJ sensitivity', '_OBJ precision',
                                '_OBJ F1']

        self.metrics_included_in_streamtable = ['Dice', 'Sensitivity', 'Precision', 'Volume Similarity',
                                                '_OBJ sensitivity', '_OBJ precision', '_OBJ F1']
    ...
    def update_streamtable(self, dict):
        ...
        st = StreamTable(table_name=task,
                 entity_name=wandb.api.viewer()['entity'],
                 project_name='Yucca')

        stream_dict = {"0. Experiment": model_name,
                       "0. Target Task": target}

        for key, _ in dict.items():
            if key == "0":
                continue
            else:
                stream_dict.update({f"{key}. "+k:v for k, v in dict[key].items()
                                    if k in self.metrics_included_in_streamtable})
        st.log(stream_dict)
        st.finish()
